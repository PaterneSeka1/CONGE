...
export default function AccountantDeptEmployeesHistory() {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/departments/daf/employees/history", {
      ...
      setRows(nextRows.filter((row) => row.employeeId !== currentEmployee?.id));
    } finally {
      setIsLoading(false);
    }
  }, [currentEmployee?.id]);
  ...
  return (
    <div className="p-6">
      <div className="text-xl font-semibold mb-1 text-vdm-gold-800">
        Historique des congés - département DAF
      </div>
